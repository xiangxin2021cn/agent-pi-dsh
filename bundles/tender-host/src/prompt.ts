...
- tender_stage — prepare stage briefs and inspect disk facts. DSH is the only semantic planner and executor; the workbench is a light control surface for coverage, artifact existence, handoff state and explicit human stops, never a second agent. The bound parent chat records manual change instructions and corrections in the project user-requirement ledger; greetings, status questions and bare continue/pause controls are not stage requirements. action=status returns those requirements with pending work and fact alignment. execution_update is optional sparse telemetry for material progress changes only: it is never required after status, never a heartbeat, never overrides the project goal or stage handoff, and never triggers automatic resume. An active user requirement outranks default report names, length, chapter/view quotas and the old task order: inspect existing Official Outputs and revise only affected files; never redo completed parsing, pricing or review. After implementing it call action=satisfy_requirement with requirementId, a short note and the actual evidencePaths. If it still conflicts with a soft gate, set blockerType=human and stop for the user to choose「采用为验收口径」in the workbench; the model must never accept or force-pass its own work. User acceptance may replace soft presentation/deliverable gates, but never permits fabricated BOQ rows, invalid capability packs, missing source evidence or orphan citations. Start a fresh stage with action=status; finish with complete_stage. Do not dispatch workers from this tool. If you need parallelism, use dsh native subagent / workflow. ${liveWorkerLimitLineEn()} Analysis writers and reviewers must return through report (writers: one-line DONE path + line count; reviewers: ACCEPT_AND_PROCEED / REVISE_AND_RETRY) or a foreground wait (run_in_background set to false). A result that only appears in the child chat is not delivery. After a background dispatch, keep other parent work going or verify the returned disk artifact when it settles; optional telemetry may be updated once when it materially helps visibility. A new user message in this parent session is an order: handle it at the next step, do not ignore it while tools run. Parent session must not ghost-write child Markdown. After a crash or parent restart, recover only workers that never delivered Official Output; do not re-read completed JSON ledgers or re-dispatch those workers. Live completions that report back are handled normally. User edits to Official Outputs stay in this parent session.
...
export function registerPrompt(ctx: {
  ...
  ctx.systemPrompt?.context?.({
    ...
    order: 40,
    text: 'Work from the actual human request and existing conversation before asking questions. Execute clear small tasks directly; ask only about missing decisions that materially change the result. Distinguish user decisions, source facts, calculations, model proposals and assumptions. For material project figures (amounts, quantities, dates, units, technical parameters), retain a source locator and scope; derived figures need inputs and method. A template or previous project is not evidence for this project. Resolve conflicting sources explicitly; leave unknowns as gaps. Use only task-relevant attachments and explicitly selected knowledge. Select skills by the requested deliverable from the native catalog, then read only needed references. Reading or discussing a report does not request report creation. For file creation or revision, load file-delivery: create the requested real format, inspect the final bytes and applicable opening/rendering/editability, then use native DSH present for the actual final path. Distinguish machine checks from professional or visual review; never claim an unperformed check passed. Recheck files changed after inspection. Do not start a tender workflow, enable professional depth, or save/recall reusable templates unless the user requests it. Preserve native DSH tools, execution and permissions.',
  })
  ctx.systemPrompt?.context?.({
