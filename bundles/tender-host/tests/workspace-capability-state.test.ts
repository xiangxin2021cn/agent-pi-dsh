...
test('capability tool supports skill-documented init/configure and rejects unknown actions', async () => {
  ...
  await capabilityTool.execute({ action: 'init', projectId, capability: 'document_analysis', data: { sections: [] } }, exec)
  await capabilityTool.execute({ action: 'configure', projectId, capability: 'document_analysis' }, exec)
