...
- `dsh-univer-office` 0.2.13 改用新 `uiConversation` 服务路径，修复启动时等待旧 `conversationEvents` 导致的 pending；安装、升级、skills 同步与卸载增加产品所有权检查，失败恢复仅覆盖应用入口和 Office。
...
- 修正原安装目录误拒绝及空目录判断；取消破坏 Office 文件清单的归档裁剪，许可构建新增实际 payload 的包内校验。Office 暂存失败时恢复应用入口，不再误称整套运行时已回滚。
...
本次修复包完成编译、包内 Office 校验和相关单元回归；按用户要求没有执行实际安装/卸载测试。安装失败时的完整 DSH/runtime 回滚尚未实现。
