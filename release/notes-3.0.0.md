...
**请手动下载本安装包。** 2.6.5 不会自动更新到 3.0。经典版可以留着，也可以与 3.0 同时装。
...
旧版在 Claude Agent SDK / Pi 之上再包一层自研 SessionManager、Goal Loop。能干活，但长任务容易多绕几圈：多一次审查、多一轮空转，时间和 token 一起涨。
...
3.0 先把发动机换对。接下来会按你们在项目上的真实用法持续自改进：把反复出现的纠正、习惯和验收标准，沉淀成**自己的经验和工作流**。让智能体越用越像你们队里的人，而不是每次从零演练。这一块指日可待。
...
（兑现：3.1.0 已实现「一句话沉淀领域模块」，见 [v3.1.0](https://github.com/xiangxin2021cn/agent-pi/releases/tag/v3.1.0)。/ Delivered in [v3.1.0](https://github.com/xiangxin2021cn/agent-pi/releases/tag/v3.1.0): one-sentence domain-module distillation.）

---

## 安装 / Install

1. 下载下面的 `Agent-Pi-DSH-3.0.0-x64.exe`
2. 未签名：SmartScreen 选「仍要运行」
3. 装好后打开 **Agent π**，选择项目工作区
4. 配置 DeepSeek；若要看图，换自定义视觉模型并打开图片理解
5. 回形针上传资料后直接下任务

1. Download `Agent-Pi-DSH-3.0.0-x64.exe` below
...
- Windows x64：`Agent-Pi-DSH-3.0.0-x64.exe`（本版先发 Windows）
- macOS / Linux：请使用 [v3.1.0](https://github.com/xiangxin2021cn/agent-pi/releases/tag/v3.1.0) —— 从 3.1 起由 CI 自动构建 dmg / AppImage / deb；或继续用经典版 [v2.6.5](https://github.com/xiangxin2021cn/agent-pi/releases/tag/v2.6.5)
- 本 Release **不**附带 `latest.yml`，2.6.5 用户不会被自动拉上 3.0

This release is **Windows x64 first**. For macOS / Linux use [v3.1.0](https://github.com/xiangxin2021cn/agent-pi/releases/tag/v3.1.0) (CI-built dmg / AppImage / deb) or Classic 2.6.5. This release does **not** ship `latest.yml`.
