...
export async function runDshPluginUpdate(plan, {
  ...
  // Local patch (agent-pi-dsh): the tender profile preinstalls this plugin as
  // a `link:` junction to the vendored offline copy. Upstream keeps `update`
  // semantics for non-registry specs, but pnpm answers "Already up to date"
  // for a link: dependency, so a registry-confirmed target must install
  // explicitly — pnpm then converts the link into a real registry install.
  // Upstream expression:
  //   target !== undefined
