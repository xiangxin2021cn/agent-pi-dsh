 ...
 */
function readDshManifest(directory, name = DSH_PACKAGE) {
    try {
...
function entryDirectories(entry) {
    if (entry === undefined)
        return [];
    const directories = [];
...
function flatDesktopHost(directory) {
    ...
    if (runtime[0] === null && runtime[1] === null)
        return null;
...
export function dshHostInfo(entry = process.argv[1]) {
    ...
    for (const directory of [...ancestors, ...applications.map(root => join(root, 'node_modules', '@deepseek-ai', 'dsh'))]) {
        ...
        if (manifest !== null)
            return { directory, version: declaredVersion(manifest) };
    }
