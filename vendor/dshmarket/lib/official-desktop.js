...
import { progress, TARGET_RE } from "./dsh-cli.js";
/**
...
function detail(error) {
    ...
    if (error === undefined)
        return 'plugin manager did not explain the failure';
...
export function createOfficialDesktopRuntime(managerLookup, profileName, _profileDirectory) {
    ...
    const runPlugin = (profile, argv) => {
        if (disposed)
            return Promise.resolve(failure('official desktop plugin runtime is disposed', 127));
        if (profile !== profileName)
            return Promise.resolve(failure('refusing to change a different profile', 127));
        if (active !== undefined)
            return Promise.resolve({ ...failure('another plugin operation is running', 127), busy: true });
        ...
        active = current;
        current.done = operation.then((result) => {
    ...
    return {
        ...
        cancelActive: () => {
            ...
            const manager = managerLookup();
            if (manager !== undefined)
                void manager.cancelInstall(active.requestId).catch(() => { });
            return true;
        },
