 ...
 */
export function probeCoreSshCommand(env) {
    ...
    // fresh answer.
    if (env === undefined && coreSshCommandProbe !== undefined)
        return coreSshCommandProbe ?? null;
    try {
        ...
        const answer = out.status === 0 && typeof out.stdout === 'string' && out.stdout.trim() !== ''
            ...
            : null;
        if (env === undefined)
            coreSshCommandProbe = answer;
        return answer;
    }
    catch {
        if (env === undefined)
            coreSshCommandProbe = null;
        return null;
    }
}
export function gitEnvForPnpm(env = process.env, coreSshCommand = probeCoreSshCommand()) {
    const out = {};
    // Blank counts as unset: an empty value is not a setting git can parse.
    const speaks = (value) => (value ?? '').trim() !== '';
    if (!speaks(env.GIT_TERMINAL_PROMPT))
 ...
 */
export function pnpmConfigEnvForArgs(pluginArgs) {
    const env = {};
 ...
 */
export function setHostPackageManager(invocation) {
    const previous = hostPackageManager;
    const unchanged = previous === invocation || (previous !== null && invocation !== null &&
...
export function hostPackageManagerInvocation() {
    return hostPackageManager;
}
/** Why the host-supplied package manager last failed, or null. */
export function lastHostPackageManagerFailure() {
 ...
 */
function probeHostPackageManager() {
    if (hostPnpmReady)
 ...
 */
function hostPackageManagerHint() {
    const invocation = hostPackageManager;
