...
import { disableRow, enableRow } from "./patch.js";
import { addProfileBundle, removeProfileBundle } from "./profile.js";
 ...
 */
export function parseBootFailure(text) {
    ...
    for (const match of normalized.matchAll(/plugin\(s\) failed to load:/gu)) {
        if (match.index !== undefined)
            markers.push({ index: match.index, kind: 'unresolved' });
    }
    for (const match of normalized.matchAll(/\d+ entr(?:y|ies) did not activate/gu)) {
        if (match.index !== undefined)
            markers.push({ index: match.index, kind: 'inactive' });
    }
    ...
    for (const match of normalized.matchAll(/failed to apply loader entry [^\s(]+ \([^)]+\):/gu)) {
        if (match.index !== undefined)
            markers.push({ index: match.index, kind: 'apply' });
    }
    ...
    if (last.kind === 'apply') {
        ...
        for (const raw of (list?.[1] ?? '').split(',')) {
            const name = raw.trim();
            if (name !== '')
                entries.push({ name, reason: 'failed to load (module could not be resolved)', kind: 'unresolved' });
        }
    }
    else {
        ...
        for (let index = 1; index < lines.length; index += 1) {
            ...
            if (/^\s/u.test(line))
                continue;
            if (line.startsWith('[') || line.startsWith('dsh: '))
                break;
 ...
 */
export function matchFailureToPlugins(plugins, failure) {
    ...
    for (const entry of failure.entries) {
        ...
        if (hit === undefined)
            ...
            if (!reasons.has(hit.name))
                reasons.set(hit.name, entry);
        }
    }
 ...
 */
export function recoveryPayload(config, failure, lastErrors = []) {
 ...
 */
export async function applyRecovery(config, enabled) {
    ...
    for (const plugin of config.plugins) {
        ...
        if (want === plugin.enabled)
            continue;
        let error = null;
        if (plugin.carrier) {
            try {
                if (want)
                    addProfileBundle(config.profileDir, plugin.name);
                else
                    removeProfileBundle(config.profileDir, plugin.name);
            }
            catch (cause) {
...
export function portListening(port, timeoutMs = 500) {
 ...
 */
export async function respawnAndWatch(config) {
    if (config.port === null)
        return false;
    let child;
    try {
    ...
    while (Date.now() < deadline) {
        if (await portListening(config.port)) {
            ...
            else if (Date.now() - answeringSince >= settleMs)
                return true;
        }
        else {
 ...
 */
export function recoveryPageHtml(lang) {
 ...
 */
export async function startRecoveryServer(config, failure, options = {}) {
    let currentFailure = failure;
    let applyRequested = null;
    ...
    const handle = async (request, response) => {
        ...
        if (path === '/dsh-market/recovery/apply' || path === '/dsh-market/recovery/release') {
            ...
            const enabled = Array.isArray(requested) ? requested.filter((name) => typeof name === 'string') : [];
            applying = true;
    ...
    if (port === null)
        throw new Error('dsh-market recovery: no port to serve on');
    ...
    let finished = null;
    let resolveFinished = () => { };
    const finishedPromise = new Promise((resolvePromise) => { resolveFinished = resolvePromise; });
 ...
 */
export async function runRecovery(config, facts = { exitCode: null, bound: false }, options = {}) {
    ...
    for (;;) {
        ...
        if (!applied.ok) {
            ...
            for (const error of applied.errors)
                note(config, `could not write the choice: ${error}`);
...
if (invoked !== undefined) {
    ...
    if (same)
        void runRecoveryCli(process.argv.slice(2));
}
/* v8 ignore stop */
