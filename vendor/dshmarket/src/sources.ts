...
/**
 * Catalog display names often omit the npm scope (`dsh-genui` vs
 * `@omdsh-dev/dsh-genui`). pnpm's allowBuilds matcher uses the *package*
 * name, so approving only the catalog name leaves the prepare script blocked.
 * @param entry - curated registry row (name / optional owner / optional npm).
 * @returns unique candidate package names to authorize.
 */
...
export function findInstalledAlias(
  entry: { name: string; owner?: string; npm?: unknown; url: string },
  ...
  if (typeof entry.owner === 'string' && entry.owner !== '' && !entry.name.startsWith('@')) {
    ids.add(`@${entry.owner}/${entry.name}`.toLowerCase())
    ids.add(`${entry.owner}/${entry.name}`.toLowerCase())
  }
