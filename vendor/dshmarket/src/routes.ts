...
export function mountMarketRoutes(
  ...
  async function setPluginEnabled(name: string, enabled: boolean): Promise<{ ok: boolean; reason?: string }> {
    if (isTeamComponent(name) || name === 'dsh-agent-pi-compaction') return { ok: false, reason: TEAM_MANAGED_REASON }
    ...
    host.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          if (isTeamComponent(name) || name === 'dsh-agent-pi-compaction') { sendJson(response, 400, { error: isTeamComponent(name) ? TEAM_MANAGED_REASON : '内置压缩组件由对话预设引用，不能单独修改或卸载。' }); return }
          const installed = readInstalled(config.profile, activeProfileDir)
